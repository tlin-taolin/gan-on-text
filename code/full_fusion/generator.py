...
class Generator(object):
    ...
    def pretrain_step(self, sess, x):
        self.fuser.fuse(self.g_embeddings.eval(session=sess))
        candidates_tofeed = self.fuser.get_candidates_tofeed()
        # outputs = self.fuser.test_feed(sess)
        outputs = sess.run([self.pretrain_updates,
                            self.pretrain_loss,
                            self.fuser.final_features
                            ],
                            feed_dict = {  self.x: x,
                                           self.fuser.input_ph: candidates_tofeed
                                        #    self.fuser.embedding_ph: self.fuser.init_embedding
                                        #    self.fuser.embedding_ph: self.g_embeddings.eval(session=sess)
                                        }
                        )
