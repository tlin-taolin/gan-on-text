...
class BasicModel(object):
    ...
    def define_training(self):
        ...
        self.optimizer_D = tf.train.AdamOptimizer(
            self.para.LEARNING_RATE_D, beta1=self.para.LEARNING_RATE_D_BETA)
        self.optimizer_G = tf.train.AdamOptimizer(
            self.para.LEARNING_RATE_G, beta1=self.para.LEARNING_RATE_G_BETA)
    ...
    def train_step(self, sess, batch_real_x, batch_z, losses):
        ...
        for _ in range(self.para.D_ITERS_PER_G_ITERS):
            _, summaries_D, loss_real_D, loss_fake_D, loss_D = sess.run(
        ...
        # summary
        self.train_summary_writer.add_summary(summaries_G)
        self.train_summary_writer.add_summary(summaries_D)
        return losses
    ...
    def run_epoch(self, sess, real_x, z, train=False, verbose=True):
        ...
        if verbose:
            sys.stdout.write('\n')
            sys.stdout.flush()
        return losses, np.mean(losses['losses_D']), np.mean(losses['losses_G'])
