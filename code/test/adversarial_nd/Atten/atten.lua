...
function model:SentencePpl()
    self.mode="test"
    local score=torch.Tensor(self.Word_s:size(1)):fill(0)
    local num=torch.Tensor(self.Word_s:size(1)):fill(0)
      ...
      --- FOR training the target_lstm, decoder.
        for t=1,self.Word_t:size(2)-1 do
            ...
            if self.Mask_t[t]:nDimension()~=0 then
                for i=1,#output do
                    output[i]:indexCopy(1,self.Mask_t[t],torch.zeros(self.Mask_t[t]:size(1),self.params.dimension));
                end
            end
            self.store_t[t]={};
            for i=1,#output-1 do
    ...
    while true do
        ...
        while End==0 do
            ...
            if train_this_batch then
                if math.fmod(batch_n, 10) == 0 then print("Training on batch: "..batch_n) end
                self.mode="train"
                local time1=timer:time().real;
                self.Word_s=self.Word_s;
                self.Word_t=self.Word_t;
