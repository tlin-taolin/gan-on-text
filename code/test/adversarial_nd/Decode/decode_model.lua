...
base_model = require("../Atten/atten")
...
function decode_model:sample()
    ...
    local beamHistory=torch.ones(self.Word_s:size(1),batch_max_dec_length)
    for t=1,batch_max_dec_length do
        local lstm_input=self:clone_(self.last)
        table.insert(lstm_input,self.context);
        if t==1 then
            table.insert(lstm_input,torch.Tensor(self.Word_s:size(1)):fill(self.Data.EOS));
        else
        ...
        local prob=self.softmax:forward({output[#output],torch.Tensor(output[#output]:size(1)):fill(1)})
        prob=prob[2]
        prob=nn.Exp():forward(prob);
        ...
        if self.params.setting=="StochasticGreedy" then
            ...
            prob=nn.Normalize(1):forward(select_P)
            next_words_index=torch.multinomial(prob, 1);
            next_words=torch.Tensor(self.Word_s:size(1),1):fill(0)
        ...
        if end_boolean_index:sum()~=0 then
            local end_examples=torch.range(1,self.Word_s:size(1))[end_boolean_index];
            for i=1,end_examples:size(1)do
                ...
                if completed_history[example_index]==nil then
                    if t~=1 then
                        ...
                        completed_history[example_index]=torch.Tensor(1,1):fill(1)
                    end
                end
            end
        end
...
function decode_model:decode_BS()
    self:model_forward()
    local span_index1=torch.expand(torch.reshape(torch.expand(torch.reshape(torch.range(1,self.params.beam_size),self.params.beam_size,1),self.params.beam_size,self.params.beam_size),1,self.params.beam_size*self.params.beam_size),self.Word_s:size(1),self.params.beam_size*self.params.beam_size);
    -- if batch_size=3  beam_size=2 span_index1 is
    --1 1 2 2
    --1 1 2 2
    --1 1 2 2
    local span_index2=torch.expand(torch.reshape(self.params.beam_size*(torch.range(1,self.Word_s:size(1))-1),self.Word_s:size(1),1),self.Word_s:size(1),self.params.beam_size);
    -- if batch_size=3  beam_size=2 span_index2 is
    --0 0
    --2 2
    --4 4
    local span_index3=torch.expand(torch.reshape((torch.range(1,self.Word_s:size(1))-1)*self.params.beam_size*self.params.beam_size,self.Word_s:size(1),1),self.Word_s:size(1),self.params.beam_size);
    ...
    if self.params.DiverseRate~=0 and self.params.DiverseRate~=nil then
        diverse_part=self.params.DiverseRate*torch.expand(torch.reshape(torch.range(0,self.params.beam_size-1),1,self.params.beam_size),self.params.beam_size*self.Word_s:size(1),self.params.beam_size);
    end
    local scores
    local completed_history={};
    local batch_max_dec_length;
    ...
    local beamHistory=torch.ones(self.Word_s:size(1)*self.params.beam_size,batch_max_dec_length)
    for t=1,batch_max_dec_length do
        local lstm_input=self:clone_(self.last)
        if t==2 then
        ...
        local full_logP=self.softmax:forward({output[#output],torch.Tensor(output[#output]:size(1)):fill(1)})
        ...
        if t==1 then
            ...
            if end_boolean_index:sum()~=0 and t>self.params.target_length then
                local end_index=torch.range(1,reshape_words:nElement())[end_boolean_index]
                ...
                local extract_history=beamHistory:index(1,end_index_in_history);
                for i=1,end_examples:size(1)do
                    local example_index=end_examples[i];
            ...
            select_index=select_index:double()
