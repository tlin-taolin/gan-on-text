...
function model:model_forward()
  ...
  for i = 1, #self.Word_s do
    for t = 1, self.Word_s[i]:size(2) do
      ...
      if self.Mask_s[i][t]:nDimension()~=0 then
        for k = 1, #output do
          output[k]:indexCopy(1, self.Mask_s[i][t], torch.zeros(self.Mask_s[i][t]:size(1), self.params.dimension))
        end
      end
      if self.mode == "train" then
  ...
  for t = 1, #self.Word_s do
    ...
    if t == 1 then
      for ll = 1, self.params.layers do
        table.insert(input,torch.zeros(self.Word_s[1]:size(1),self.params.dimension))
        table.insert(input,torch.zeros(self.Word_s[1]:size(1),self.params.dimension))
      end
    else
      if self.mode == "train" then
...
function model:model_backward()
  ...
  if self.mode == "train" then
      local dh = self.softmax:backward(
        ...
        {torch.Tensor({1}),
        torch.Tensor(softmax_output[2]:size()):fill(0)}
      )
      local d_words = {}
      local d_output = {}
      for ll = 1,self.params.layers do
          table.insert(d_output,torch.zeros(self.Word_s[1]:size(1), self.params.dimension))
          table.insert(d_output,torch.zeros(self.Word_s[1]:size(1), self.params.dimension))
      end
...
function model:test()
  ...
  while End == 0 do
    ...
    -- self.Word_s[1] -- questions (+ and -)
    -- self.Word_s[2] -- answerings (+ and -)
    if End == 1 then break end
    self:model_forward()
...
function model:lstm_(isWordLevel)
  local inputs = {} -- a collection of h and c
  local outputs = {}
  for ll = 1, self.params.layers do
  ...
  for ll = 1, self.params.layers do
    local prev_h = inputs[2*ll-1]
    local prev_c = inputs[2*ll]
    local x
    if ll == 1 then
    ...
    local drop_h = nn.Dropout(self.params.dropout)(inputs[2*ll-1])
    local i2h = nn.Linear(self.params.dimension, 4*self.params.dimension)(drop_x) --input2h
