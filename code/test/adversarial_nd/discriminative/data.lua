...
function Data:read_train(open_pos_train_file, open_neg_train_file)
  ...
  while 1==1 do
    ...
    for k = 1, #strings do -- 2
      Source[k][i] = split(stringx.strip(strings[k]))
    end
    labels[i] = 1
    if i == self.params.batch_size/2 then break end
  end
  ...
  while 1==1 do
    ...
    labels[i] = 2
    if i == self.params.batch_size then break end
  end

  if End == 1 then -- finished with abnormal
    return End, {}, {}, {}, {}
  end

  local Words_s, Mask_s, Left_s, Padding_s = self:get_batch(Source)
