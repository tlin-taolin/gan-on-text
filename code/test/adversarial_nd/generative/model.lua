...
function model:Initial(params)
    self.params=params
    self.generate_model=require("./generative")
    ...
    self.disc_model=require("../discriminative/dis_model")
    print("Opening")
    print(self.params.disc_params)
...
function model:TrainD(open_train_file)
    ...
    local Source={};
    Source[1]={};Source[2]={}
    self.disc_model.labels=torch.Tensor(#self.generate_model.Source*2)
    ...
    self.disc_model.labels=self.disc_model.labels
...
function model:MonteCarloReward()
    ...
    for t=1,self.generate_model.Word_t:size(2)-2 do
        ...
        self.disc_model.mode="test"
        self.disc_model:model_forward()
        self.disc_model.labels=torch.Tensor(self.disc_model.Word_s[1]:size(1)):fill(1)
        local dis_pred=self.disc_model:model_backward()
        dis_pred=torch.exp(dis_pred)
