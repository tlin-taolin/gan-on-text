local model={}
local base_model=require("../Decode/decode_model")
...
function model:GenerateSample()
    ...
    if self.params.sample then
        ...
        for i,v in pairs(self.sample_target)do
            self.sample_target[i]=torch.cat(torch.Tensor(1,1):fill(self.Data.EOS),torch.cat(v,torch.Tensor(1,1):fill(self.Data.EOT),2),2)
        end
    else
...
--sampling for partially decoded sequences until the end
    ...
    for t=1,self.params.max_length-current_time do
        ...
        local _,prob=unpack(self.softmax:forward({output[#output],torch.Tensor(output[#output]:size(1)):fill(1)}))
        ...
        for i=1,starting_word:size(1)do
            if finish[i]==nil then
                if starting_word[i]==self.Data.EOT or (self.partial_history[i]:eq(self.Data.EOT)):sum()~=0 then
                    ...
                    self.partial_history[i]=torch.cat(self.partial_history[i],torch.Tensor(1,1):fill(starting_word[i]))
                end
            end
        end
