...
opt.geometry = {3, opt.scale, opt.scale} -- rgb x 64 x 64 images
local input_sz = opt.geometry[1] * opt.geometry[2] * opt.geometry[3]

if opt.network == '' then -- if not load pretrained
  ----------------------------------------------------------------------
  ...
  model_D:add(nn.SpatialMaxPooling(2,2))
  model_D:add(nn.SpatialDropout(0.2))
  model_D:add(nn.Reshape(8*8*64))
  model_D:add(nn.Linear(8*8*64, 1024))
  model_D:add(nn.ReLU(true))
  ...
  x_input = nn.Identity()()
  lg = nn.Linear(opt.noiseDim, 64*8*8)(x_input)
  lg = nn.Reshape(64, 8, 8)(lg)
  lg = nn.ReLU(true)(lg)
  lg = nn.SpatialUpSamplingNearest(2)(lg)
  lg = nn.SpatialConvolution(64, 128, 5, 5, 1, 1, 2, 2)(lg)
  lg = nn.SpatialBatchNormalization(128)(lg)
  lg = nn.ReLU(true)(lg)
  lg = nn.SpatialUpSamplingNearest(2)(lg)
  lg = nn.SpatialConvolution(128, 128, 5, 5, 1, 1, 2, 2)(lg)
  lg = nn.SpatialBatchNormalization(128)(lg)
  lg = nn.ReLU(true)(lg)
  lg = nn.SpatialUpSamplingNearest(2)(lg)
  lg = nn.SpatialConvolution(128, 64, 5, 5, 1, 1, 2, 2)(lg)
  lg = nn.SpatialBatchNormalization(64)(lg)
  lg = nn.ReLU(true)(lg)
  lg = nn.SpatialConvolution(64, 3, 3, 3, 1, 1, 1, 1)(lg)
