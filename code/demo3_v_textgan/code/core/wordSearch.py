...
class WordSearch(object):
    def __init__(self, vocab, para):
        self.vocab = vocab
        self.beam_candidates = []
        self.beam_options = []
        self.best_sequence = [vocab.get('go')]
    ...
    def beam_search(
            self, sess, placeholder_x, placeholder_g_state,
            top_value, top_index, state):
        for i in range(len(self.beam_candidates)):
            cur_word_score = self.beam_candidates[i][0]
            cur_word_indices = self.beam_candidates[i][1]

            if len(cur_word_indices) > self.para.SAMPLING_LENGTH or \
                    cur_word_indices[-1] == self.vocab.get('eos'):
        ...
        for i in range(
                min(len(self.beam_options), self.para.BEAM_SEARCH_SIZE)):
